...
*/
    ...
    function checkViewCache()
    {
         ...
         if($cacheType   = $cacheInfo['type']) {
             ...
             if($cacheType=='ALL') {
                ...
                if(file_exists($cacheFile) && NOW <= filemtime($cacheFile)+$cacheTime) {
                    // 视图缓存有效 读取缓存Action
                    readfile($cacheFile);
                    exit();
                }
    ...
    {
        //搜索是否有IXCoreCache标签 判断视图缓存类型 <ixcorecache type="" time="" />
        $find = preg_match('/<ixcorecache\s(.+?)\s\/>\W/is',$content,$matches);
        if($find) {
            //替换IXCoreCache标签
            ...
            if($cacheInfo['default'] == 'NOCACHE') {
                ...
                if($find) {
                    ...
                    foreach($matches as $key=>$match) {
                        ...
                        $xml =  '<ixcore><cache '.$cacheAttr.' /></ixcore>';
                        $xml = simplexml_load_string($xml);
