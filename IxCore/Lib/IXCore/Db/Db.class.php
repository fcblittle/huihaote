...
class Db extends Base
{
    ...
    // 数据库表达式
    protected $comparison      = array('=','!=','>','>=','<','<=','NOT LIKE','LIKE','BETWEEN','IS NOT NULL','IS NULL');
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function parseWhere($where, $return = true)
    {
        $whereStr = '';
        if(is_string($where) || is_null($where)) {
            //支持String作为条件 如使用 > like 等，请降低使用率。
            $whereStr = $where;
            ...
            foreach ($where as $key=>$val){
                if(is_array($val) && array_key_exists('_logic',$val))
                {
                    $whereStr .= "( {$this -> parseWhere($val, false)} ){$operate}";
                }else{
                    if(strpos($key,C('FIELDS_DEPR'))) {
                        $key    =   explode(C('FIELDS_DEPR'),$key);
                        array_walk($key, array($this, 'addSpecialChar'));
                    }else{
                        $key = (!is_numeric($key))?$this->addSpecialChar($key):$key;
                    }
                    $whereStr .= "( ";
                    if(is_array($val) && is_string($val[0])) {
                        $val[0] = strtoupper($val[0]);
                        if(in_array($val[0], $this->comparison)) {
                            // 是否存在比较运算
                            $whereStr .= "{$key} {$val[0]} {$this->fieldFormat($val[1])}";
                        }elseif($val[0] == 'IN'){
                            // 支持 IN 运算
                            $zone   =   is_array($val[1])? implode(',',$val[1]):$val[1];
                            $whereStr .= "{$key} IN ({$zone})";
                        }elseif(in_array(substr($val[0],0,-1), $this->comparison)){
                            $whereStr .= "{$key} {$val[0]} {$val[1]}";
                        }else{
                            $whereStr .= "( {$this -> parseWhere($val, false)} ) AND ";
                        }
        ...
        $whereprefix = $return?' WHERE ':'';
        return empty($whereStr)?'':$whereprefix.$whereStr;
    }
    ...
    protected function parseSets($sets)
    {
        ...
        if(is_array($sets)){
            foreach ($sets as $key=>$val){
                ...
                if(is_array($val) && isset($val[0]) && strtolower($val[0]) == 'exp') {
                    $val    =   $val[1];                        // 使用表达式
    ...
    {
        $queryIps = array('INTO','INSERT','UPDATE','DELETE','REPLACE','CREATE','DROP','LOAD DATA','COPY','ALTER','GRANT','REVOKE','LOCK','UNLOCK');
        foreach ($queryIps as $value)
        {
            $check = strripos($query,$value);
            if($check!==false && ($value!='INTO' || preg_match('/^\s*"?(SELECT .* INTO)\s+/i', $query))){
                return true;
            }
        }
        return false;
    ...
    public function lazyQuery($sql='') {
        ...
        import("IXCore.Db.ResultIterator");
    ...
    public function find($where,$tables,$fields='*',$order=null,$limit=null,$group=null,$having=null,$join=null,$cache=false,$lazy=false,$lock=false)
    {
        switch($this->getDbType())
		{
			...
			case 'ORACLE':
				if($limit)
				{
					$this->queryStr = "SELECT *	FROM (SELECT rownum AS numrow, ixcorephp.* FROM (SELECT "
						.$this->parseFields($fields)
						." FROM ".$tables
						.$this->parseJoin($join)
						.$this->parseWhere($where)
