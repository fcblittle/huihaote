...
{//类定义开始
    ...
    static function setExpire($time, $add = false)
    {
        if ($add) {
            if (!isset($_SESSION['__HTTP_Session_Expire_TS'])) {
                $_SESSION['__HTTP_Session_Expire_TS'] = NOW + $time;
            }
    ...
    static function setIdle($time, $add = false)
    {
        if ($add) {
            ...
            $_SESSION['__HTTP_Session_Idle'] = $time - NOW;
        }
    }
    ...
    {
        if (isset($_SESSION['__HTTP_Session_Expire_TS']) && $_SESSION['__HTTP_Session_Expire_TS'] < NOW) {
            return true;
        } else {
            return false;
        }
    ...
    {
        if (isset($_SESSION['__HTTP_Session_Idle_TS']) && (($_SESSION['__HTTP_Session_Idle_TS'] + $_SESSION['__HTTP_Session_Idle']) < NOW)) {
            return true;
        } else {
            return false;
        }
    ...
    {
        $_SESSION['__HTTP_Session_Idle_TS'] = NOW;
    }
