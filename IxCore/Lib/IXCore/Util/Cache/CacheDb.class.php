...
{//类定义开始
    ...
    public function get($name)
    {
		...
		$this->Q(1);
        $result  =  $this->db->getRow('select `data`,`datacrc`,`datasize` from `'.$this->options['table'].'` where `cachekey`=\''.$name.'\' and (`expire` =-1 OR `expire`>'.NOW.') limit 0,1');
        if(false !== $result ) {
            if(is_object($result)) {
    ...
    public function set($name, $value,$expireTime=0)
    {
        ...
        $map['data']	=	$data	 ;
        $map['datacrc']	=	$crc;
        $map['expire']	=	($expire==-1)?-1: (NOW+$expire) ;//缓存有效期为－1表示永久缓存
