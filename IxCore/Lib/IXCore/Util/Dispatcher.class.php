...
{//类定义开始
    ...
    static function dispatch()
    {
        ...
        if($urlMode == URL_REWRITE ) {
            ...
            if(!C('HTML_URL_SUFFIX')){
                ...
                define('PHP_FILE', substr(_PHP_FILE_, 0, -$len));
            }
        }else {
            //当前项目地址
        ...
        if($urlMode == URL_PATHINFO || $urlMode == URL_REWRITE || $urlMode == URL_COMPAT) {
            ...
            if(($urlMode == URL_REWRITE || $urlMode == URL_COMPAT) && C('HTML_URL_SUFFIX')){
                $count = count(explode('/', trim($_SERVER['PATH_INFO'],'/')));
                $check = ($count > 2) ? true : false; // 可根据URL调整
            }
            if ( ((!isset($check) && !empty($_GET)) || (isset($check) && $check)) && !isset($_GET[C('VAR_PATHINFO')]) && !isset($_GET[C('VAR_ROUTER')])) {
                ...
                if(isset($check)) {
                    ...
                    $_URL .= C('HTML_URL_SUFFIX').'?';
                    foreach ($_GET as $_VAR => $_VAL) {
                        if('' != trim($_GET[$_VAR])) {
                            $_VAL = rawurlencode($_VAL);
                            $_URL .= "{$_VAR}={$_VAL}&";
                        }
                    }
                    $_URL = substr($_URL, 0, -1);
    ...
    private static function routerCheck() {
        ...
        if(file_exists_case(CONFIG_PATH.'routes.php')) {
            ...
            if(C('HTML_URL_SUFFIX')) {
                ...
                $_SERVER['PATH_INFO']   =   str_replace($_SERVER['PATH_INFO'],".{$suffix}", '');
            }
            if(isset($_GET[C('VAR_ROUTER')])) {
                // 存在路由变量
    ...
    private static function getPathInfo()
    {
        ...
        if(!empty($_SERVER['PATH_INFO'])) {
            if(C('HTML_URL_SUFFIX')) {
                $suffix =   substr(C('HTML_URL_SUFFIX'),1);
                $_SERVER['PATH_INFO']   = str_replace(".{$suffix}", '', $_SERVER['PATH_INFO']);
            ...
            if(C('PATH_MODEL')==2){
                ...
                $fun = (get_magic_quotes_gpc())?'addslashes': '';
                for($i = 0, $cnt = count($paths); $i <$cnt; $i++){
                    if(isset($paths[$i+1])) {
                        $pathInfo[$paths[$i]] = $fun ? (string)$fun($paths[++$i]) : $paths[++$i];
                    }elseif($i==0) {
                        $pathInfo[$pathInfo[C('VAR_ACTION')]] = $fun ? (string)$fun($paths[$i]) : $paths[$i];
                    }
                }
            }
            else {
