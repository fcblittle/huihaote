 ...
 * @version  $Id$
 +------------------------------------------------------------------------------
 */
class  IXCoreTemplate extends Base
{//类定义开始
    ...
    public function loadTemplate ($tmplTemplateFile='',$charset='')
    {
        ...
        if (!$this->checkCache($tmplTemplateFile)) {
            // 需要更新模版 读出原模板内容
            $tmplContent = file($tmplTemplateFile);
            foreach ($tmplContent as $k => $v){
                $tmplContent[$k] = trim($v, "\t "); // 有去除问题
            }
            $tmplContent = implode('', array_filter($tmplContent));
            //编译模板内容
    ...
    protected function checkCache($tmplTemplateFile)
    {
        ...
        if(!file_exists($tmplCacheFile)){
            ...
            // 模板文件如果有更新则缓存需要更新
            return false;
        }elseif (C('TMPL_CACHE_TIME') != -1 && NOW > filemtime($tmplCacheFile)+C('TMPL_CACHE_TIME')) {
            // 缓存是否在有效期
            return false;
        }
        //缓存有效
    ...
    public function parse($content)
    {
        ...
        // 标签库只需要定义一次，允许引入多个一次
        // 一般放在文件的最前面
        // 格式：<taglib name="cx,html" class="IXCore.Util.TagLib.TagLib_Cx,IXCore.Util.TagLib.TagLib_Html" />
        ...
        if(!empty($this->tagLib)) {
            ...
            foreach($this->tagLib as $tagLibName=>$tagLibClass) {
                if(empty($tagLibClass)) {
                    import('IXCore.Template.TagLib.TagLib'.ucwords(strtolower($tagLibName)));
                }else {
                    import($tagLibClass);
                }
        ...
        // 的形式
        import('IXCore.Template.TagLib.TagLibCx');
    ...
    public function parseTag($tagStr){
        ...
        switch(strtoupper($tag)){
            case 'INCLUDE':
                ...
                break;
            case 'NOTE':       //note 节点标签，待纠正
                $parseStr = $this->parseInclude(DATA_PATH.'Note/'.MODULE_NAME.'_'.ACTION_NAME.'_Template_'.$note.'.html');
                break;
            //这里扩展其它标签
            //…………
            default:
    ...
    public function parseVar($varStr){
        ...
        if(!empty($varStr)){
            ...
            if('IXCore.' == substr($var,0,7)){
                // 所有以IXCore.打头的以特殊变量对待 无需模板赋值就可以输出
                $name = $this->parseIXCoreVar($var);
            }
     ...
     * @return string
     +----------------------------------------------------------
     */
    public function parseIXCoreVar($varStr){
        $vars = explode('.',$varStr);
        ...
        if(count($vars)>=3){
            ...
            switch($vars[1]){
                case 'NOW':
                    $parseStr = "date('Y-m-d g:i a',NOW)";break;
                case 'VERSION':
                    $parseStr = 'IXCORE_VERSION';break;
                case 'TEMPLATE':
    ...
    public function parseInclude($tmplPublicName){
        if(is_file($tmplPublicName)) {
            ...
            $parseStr = file($tmplTemplateFile);
        }
        foreach ($parseStr as $k => $v) {
            $parseStr[$k] = trim($v, "\t ");// 有去除末尾\问题
        }
        $parseStr = implode('', array_filter($parseStr));
        //再次对包含文件进行模板分析
