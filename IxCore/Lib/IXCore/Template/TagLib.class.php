 ...
 *
 * 如果要指定标签库解析类，可以使用
 * <taglib name='cx'  class='IXCore.Template.TagLib.TagLib_cx' />
...
{//类定义开始
    ...
    public function __construct($tagLib='',$filename='')
    {
        ...
        $this->tagLib  = $tagLib;
        $this->tpl       = IXCoreTemplate::getInstance();
        if(!empty($filename)) {
            $this->xml = $filename;
        }else {
    ...
    public function autoBuildVar($name) {
        if('IXCore.' == substr($name,0,7)){
            // 特殊变量
            return $this->parseIXCoreVar($name);
        }elseif(strpos($name,'.')) {
            // 数组和对象自动判断支持
            $vars = explode('.',$name);
    ...
    public function parseIXCoreVar($varStr){
        ...
        if(count($vars)==3){
            ...
            switch($vars[1]){
                case 'NOW':       $parseStr = "date('Y-m-d g:i a',NOW)";break;
                case 'VERSION':  $parseStr = 'IXCORE_VERSION';break;
