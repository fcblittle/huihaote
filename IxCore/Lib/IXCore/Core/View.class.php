   ...
   /**
    ...
    public function layout($layoutFile='',$charset='',$contentType='text/html',$varPrefix='',$display=true)
    {
        ...
        if($display) {
            ...
            echo $content;
            if(C('SHOW_RUN_TIME')) {
                echo '<div  id="ixcore_run_time" class="ixcore_run_time">'.$showTime.'</div>';
            }
    ...
    public function fetch($templateFile='',$charset='',$contentType='text/html',$varPrefix='',$display=false)
    {
        ...
        }elseif('IXCORE'==$this->type){
            // 使用内置的IXCoreTemplate模板引擎
            if(!$this->checkCache($templateFile)) {
                // 缓存无效 重新编译
                $compiler   =   true;
                import('IXCore.Template.IXCoreTemplate');
                $tpl = IXCoreTemplate::getInstance();
    ...
    protected function showTrace($showTime,$charset,$compiler=true){
        if(C('SHOW_PAGE_TRACE')) {
            ...
            // 调用Trace页面模板
            include IXCORE_PATH.'/Tpl/PageTrace.tpl.php';
        }
    }
}//
?>