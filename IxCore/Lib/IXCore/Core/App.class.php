...
{//类定义开始
    ...
    public function init()
    {
        ...
        // 例如 ?m=user&a=add
		if(C('DISPATCH_ON')) {
			if( 'IXCore'== C('DISPATCH_NAME') ) {
				// 使用内置的IXCoreDispatcher调度器
				import('IXCore.Util.Dispatcher');
				Dispatcher::dispatch();
			}elseif(C('IXCORE_PLUGIN_ON')) {
				// 加载第三方调度器
		...
		if(C('LIMIT_RESFLESH_ON') && (!isset($_REQUEST[C('VAR_RESFLESH')]) || $_REQUEST[C('VAR_RESFLESH')]!="1")) {
			//	启用页面防刷新机制
			$guid	=	md5($_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);
			// 检查页面刷新间隔
			if(Cookie::is_set('_last_visit_time_'.$guid) && Cookie::get('_last_visit_time_'.$guid)>NOW-C('LIMIT_REFLESH_TIMES')) {
				// 页面刷新读取浏览器缓存
	...
	{
		// 加载惯例配置文件
		C(array_change_key_case(include IXCORE_PATH.'/Common/convention.php'));
		...
		if(C('DEBUG_MODE')) {
			// 加载系统默认的开发模式配置文件
			C(array_change_key_case(include IXCORE_PATH.'/Common/debug.php'));
    ...
    private function checkLanguage()
    {
		if(C('LANG_SWITCH_ON')) {
			...
			if(isset($_GET[C('VAR_LANGUAGE')])) {
				...
				// 记住用户的选择
				Cookie::set('ixcore_language',$langSet,NOW+3600);
			}elseif ( Cookie::is_set('ixcore_language') ) {
				// 获取上次用户的选择
				$langSet = Cookie::get('ixcore_language');
			}else {
			...
			if(C('LANG_CACHE_ON') && file_exists(TEMP_PATH.MODULE_NAME.'_'.LANG_SET.'_lang.php')) {
				...
				// 加载框架语言包
				if (file_exists_case(IXCORE_PATH.'/Lang/'.LANG_SET.'.php')){
					L(include IXCORE_PATH.'/Lang/'.LANG_SET.'.php');
				}else{
					L(include IXCORE_PATH.'/Lang/'.$defaultLang.'.php');
				}
			...
			// 不使用语言包功能，仅仅加载框架语言文件
			L(include IXCORE_PATH.'/Lang/'.C('DEFAULT_LANGUAGE').'.php');
		}
		return ;
    }
    ...
    private function checkTemplate()
    {
		if(C('TMPL_SWITCH_ON')) {
			...
			if ( isset($_GET[$t]) ) {
				$templateSet = $_GET[$t];
				Cookie::set('ixcore_template',$templateSet,NOW+3600);
			} else {
				if(Cookie::is_set('ixcore_template')) {
					$templateSet = Cookie::get('ixcore_template');
				}else {
    ...
    {
        // 加载插件必须的函数
        include IXCORE_PATH.'/Common/plugin.php';
        //加载有效插件文件
		if(file_exists(RUNTIME_PATH.'~plugins.php')) {
			include RUNTIME_PATH.'~plugins.php';
		}else{
			// 检查插件数据
			$common_plugins = get_plugins(IXCORE_PATH.'/PlugIns','IXCore');// 公共插件
