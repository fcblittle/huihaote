...
{
    protected $mname = 'Fitting';   //模块名称

    protected $type = 'fitting';
    ...
    public function index()
    {
		...
		if($_GET['p'])
            Cookie::set('fittingPage', $_GET['p']);
        else
            Cookie::set('fittingPage', 1);
            ...
            foreach($_POST['material']['goods'] as $key => $value)
            {
                if($value && (float)$_POST['material']['used'][$key])
                    $material[$value] = array(
                        'goods'     => $value,
                        'used'      => abs((float)$_POST['material']['used'][$key]),
    ...
    protected function pass()
    {
        ...
        foreach ($_used as $goods => $val)
        {
            ...
            foreach($_POST['out_group'][$goods] as $k=>$v)
            {
                ...
                //$_used[$goods]['stock'][] = array('group'=>$v, 'num'=>abs($_POST['out_num'][$goods][$k]));

                //耗费信息
                $num = $_POST['out_num'][$goods][$k];

                $productCost = A("Order") -> productCost($val['goods'], $num, $v);

                $cost['total'] += $productCost['price'];
                //销售的产品 购买时的入库信息
                $cost['stock'][$val['goods']][$v] = array(
                ...
                $stock[] = array(
                    ...
                    'price'     => (int)abs($productCost['price']),
                    'time'      => $order['time'],
                    'uid'       => Session::get('uid'),
                    'rtime'     => NOW,
                    'comment'   => '组装--原材料--'.$val['com'],
                    'order'     => $order['id'],
                    'come'      => 'fitting',
                );
                $_used[$goods]['stock'][] = array('group'=>$v, 'num'=>abs($_POST['out_num'][$goods][$k]), 'price'=>(int)abs($productCost['price']));
            }
        }

        //获取拆分成的产品的总数
        $in_num = array_sum(array_map(function($val){return $val['0'];}, $_POST['in_num']));
        $perprice = (int)abs($cost['total']/$in_num);
        ...
        foreach ($_goods as $goods => $val)
        {
            ...
            {
                $_goods[$goods]['stock'][] = array('group'=>$v, 'num'=>abs($_POST['in_num'][$goods][$k]), 'price' => -(abs($_POST['in_num'][$goods][$k]) * $perprice));
                $stock[] = array(
                    'goods'     => $goods,
                    'goods_name'=> $_good['name'],
                    'group'     => $v,
                    'num'       => abs($_POST['in_num'][$goods][$k]),
                    'price'     => -(abs($_POST['in_num'][$goods][$k]) * $perprice),
                    'time'      => $order['time'],
                    'uid'       => Session::get('uid'),
                    'rtime'     => NOW,
                    'comment'   => '组装--成品--'.$val['com'],
                    'order'     => $order['id'],
                    'come'      => 'fitting',
                );
            }
        ...
        if($DB_Order -> save(array('audit'=>Session::get('uid'), 'used'=>serialize($_used), 'goods'=>serialize($_goods), 'coststock'=>serialize($cost['stock'])), "`id`='{$order['id']}'"))
        {
            $order['audit'] = 1;
