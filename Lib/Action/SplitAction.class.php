    ...
    protected function pass()
    {
        ...
        foreach($_goods as $goods => $val)
        {
            ...
            foreach($_POST['out_group'][$goods] as $k=>$v)
            {
                ...
                $_goods[$goods]['stock'][] = array('group'=>$v, 'num'=>-abs($_POST['out_num'][$goods][$k]), 'price'=>(int)abs($productCost['price']));
        ...
        foreach ($_used as $goods => $val)
        {
            ...
            {
                $_used[$goods]['stock'][] = array('group'=>$v, 'num'=>abs($_POST['in_num'][$goods][$k]),'price' => -(abs($_POST['in_num'][$goods][$k]) * $perprice));
                $stock[] = array(
                    'goods'     => $goods,
                    'goods_name'=> $_good['name'],
                    'group'     => $v,
