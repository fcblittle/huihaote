...
class PorderAction extends OrderAction
{
    ...
    public function statistics()
    {
        ...
        if(isset($_GET['ac']) && $_GET['ac']=='打 印')
        {
            ...
            $data['list'] = $DB_Order -> xfindAll($condition, 'id,num,cors,type,uid,time,audit,over,sale,total,tax_total,income,tax', 'time Desc');
        }else{
		...
		$_allUsers = D('user') -> field('uid, name') -> where("`del` = 0") -> findAll();
		$allUsers = array();
    ...
    public function swap()
    {
        ...
        $DB_Return -> link('supplier', 'HAS_ONE', 'cors', 'supplier', 'id', 'id,name');
            ...
            */
            ...
            foreach ($_POST['goods'] as $key => $val)
            {
                ...
                $goods[$val] = array(
                 ...
                 $total += $_POST['good_num'][$key] * $_POST['good_price'][$key];
                 $cost += $_POST['good_num'][$key] * $_POST['good_cost'][$key];
