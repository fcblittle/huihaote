...
class SorderAction extends OrderAction
{
    ...
    public function statistics()
    {
        ...
        if(isset($_GET['ac']) && $_GET['ac'] != '搜 索')
        {
            ...
            $data['list'] = $DB_Order -> xfindAll($condition, 'id,num,cors,type,uid,time,audit,over,sale,total,tax_total,income,tax,cost', 'time Desc');
        }else{
		...
		$_allUsers = D('user') -> field('uid, name') -> where("`del` < 1") -> findAll();
		$allUsers = array();
    ...
    public function swap()
    {
        ...
        $DB_Return -> link('client', 'HAS_ONE', 'cors', 'client', 'id', 'id,name');
        
        import('Extend.Util.Page');
        $Page = new Page($DB_Return->count("`type`='{$this->type}'"), 15);
        $limit =  $Page->firstRow.','.$Page->listRows;

        $list = $DB_Return -> xfindAll("`type`='{$this->type}'", '*', 'rtime desc', $limit);
    ...
    public function auditSwap()
    {
        if($_POST)
        {
            ...
            $DB -> link('', 'HAS_ONE', 'num', 'order', 'num', 'coststock');
            $return = $DB -> xfind("`id` = '{$id}'");

            //销售成本明细
            $costDetail = unserialize($return['coststock']);
                    ...
                    foreach($value as $v)
                    {
                        ...
                        if($num)
                        {
                            ...
                            foreach ($_costDetial as $detail) 
                            {
                                ...
                                $cost += $detail['num'] * abs($detail['price']);
                                $__costDetail[$detail['id']] = $detail;
            ...
            $result = D('Return') -> save(array('audit'=>Session::get('uid'), 'cost'=>$cost, 'coststock'=>serialize($coststock)), "`id`='{$return['id']}'");
            if(false !== $result)
            {
                $order['audit'] = 1;
    ...
    public function viewSwap()
    {
        ...
        //订单成本核算
        $order = D("Order") -> field("coststock") -> where("`num`='{$return['num']}'") -> find();
        $this -> assign('stockDetial', unserialize($order['coststock']));
