...
class ServiceAction extends Action
{
    ...
    public function printStatistics()
    {
        ...
        //下面是新添加的把goods信息反序列化，以前没有
        foreach ($data['list'] as $key => $value) {
            //var_dump($list[$key]);exit;
            $goods = unserialize($value['goods']);
            reset($goods);
            $current = current($goods);
            $tax = $current['tax'];
            $data['list'][$key]['tax'] = $tax;
        }
        //供应商.客户
        if($this -> abs)
        {
        ...
        if($_GET['ac'] == 'export')
        {
            ...
            foreach($data['list'] as $key=>$val)
            {
                ...
                $list[$key]['salename'] = $val['sorts']['intro'];
                $list[$key]['totalPrice'] = showPrice($val['total']+$val['tax_total']);
                $list[$key]['tax'] = $val['tax'].'%';
                $list[$key]['income'] = showPrice($val['income2']);
                $list[$key]['over'] = $val['over'] ? '完结' : '未结';
            }

            $titles = array('A'=>'单 号', 'B'=>'客 户', 'C'=>'维修人', 'D'=>'金 额', 'E'=>'税 率', 'F'=>'提 成', 'G'=>'状态');
    ...
    protected function pass()
    {
        ...
        foreach($group as $key => $value)
        {
            ...
            foreach($value as $k=>$v)
            {
                ...
                if($num)
                {
                    ...
                    //计算成本
                    $productCost = A('Order') -> productCost($v['goods'], $num, $key);
                    $cost['total'] += $productCost['price'];
                    //$cost['stock'][$v['goods'].'-'.$num.'-'.$key] = $productCost['stock'];
                    //销售的产品 购买时的入库信息
        ...
        $DB_Order = D('Service');
        if($DB_Order -> save(array('audit'=>Session::get('uid'), 'express'=>$_POST['express'], 'code'=>$_POST['code'], 'comment'=>$_POST['comment'], 'sort1'=>$_POST['sort1'],  'income1'=>$_POST['income1']*100, 'sort2'=>$_POST['sort2'],  'income2'=>$_POST['income2']*100, 'cost'=>$cost['total'], 'coststock'=>serialize($cost['stock'])), "`id`='{$order['id']}'"))
        {
            $order['audit'] = 1;
    ...
    public function getlist($condition = '')
    {
        ...
        //含税维修
        $havetax = D("Service") 
                    ->  query("SELECT SUM(`total`+`tax_total`) AS havatax FROM `{$prefix}service` WHERE {$condition} AND `tax_total` > 0");
        $havetax = $havetax[0]['havatax'];
        $this -> assign('havetax',$havetax);
        $DB_Order = D('Service', true);
        $DB_Order -> link('user', 'HAS_ONE', 'uid', 'user', 'uid', 'uid,name');
        $DB_Order -> link('cors', 'HAS_ONE', 'cors', $this->abs ? 'Supplier' : 'Client', 'id', 'id,name');
        $DB_Order -> link('sorts', 'HAS_ONE', 'sort1', 'System_config', 'id', 'id,intro');
