...
class StockAction extends Action
{
    ...
    public function audit()
    {
        ...
        if($DB_Stock -> save(array('audit'=>Session::get('uid')), "`id`='{$id}'"))
        {
            //添加应收/应付款
            if($data['order'])
            {
    ...
    public function auditall()
    {
        ...
        foreach($data as $key => $value)
        {
            ...
            if($DB_Stock -> save(array('audit'=>Session::get('uid'), 'id'=>$value['id'])))
            {
    ...
    public function getlist($condition = '', $page=false)
    {
        ...
        $list = $DB_Stock -> xfindAll($condition, 'id,goods,goods_name,group,num,price,time,uid,rtime,audit,money', 'id desc', $limit);
