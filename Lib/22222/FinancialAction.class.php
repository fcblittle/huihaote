...
class FinancialAction extends Action
{
    ...
    public function index()
    {
        ...
        {
            $list = $DB_Financial -> xfindAll($condition, 'id,uid,name,price,notto,audit,time,income', 'time desc, id desc');
            ...
            $list = $DB_Financial -> xfindAll($condition, 'id,uid,name,price,notto,audit,time,income,bill', 'time desc, id desc', $Page->firstRow.','.$Page->listRows);
            $page = $Page -> show();
    ...
    public function add()
    {
        if($_POST)
        {
            ...
            $data = array(
                ...
                'num'      => $_POST['num'],
                'way'      => $_POST['way'],
                'bill'     => in_array($_POST['bill'], array('已开', '未开')) ? $_POST['bill'] : '未开',
    ...
    public function addDaily()
    {
        if($_POST)
        {
            ...
            if($DB -> find("`num` = '{$_POST['num']}' AND `type`='{$type}'"))
            {
                $postNum = explode('-', $_POST['num']);
                $num_prefix = $postNum[0].'-'.substr($postNum[1], 0, 8);
    			$expenses = $DB -> order("`num` desc") -> find("`type`='{$type}' AND `num` LIKE '$num_prefix%'");
    			$ids = explode('-', $expenses['num']);
    		    $seq = substr($ids[1], 8);
    		    $seq = $seq ? str_pad((int)$seq+1, 4, '0', STR_PAD_LEFT) : '0001';
    		    $_POST['num'] = $num_prefix.$seq;
            ...
            if(false !== $result)
            {
                ...
                $this -> success("操作成功！", 1, 'Financial/'.$type);
            }else{
                $this -> error("操作失败！");
            }
        }
