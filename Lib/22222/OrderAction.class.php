    ...
    public function add()
    {
        ...
        if($_POST)
        {
            ...
            foreach($_POST['goods'] as $key => $value)
            {
                ...
                {
                    $price = inputPrice($this -> price ? $_POST['total_price'][$key] : 0);
...
//dump($cors);exit;
    ...
    protected function pass($table = 'Order')
    {
        ...
        $_order_goods = unserialize($order['goods']);
        $order_group = array();
        foreach($_order_goods as $v)
        {
            $order_group[$v['goods']] = $v;
        }
        ...
        foreach($group as $key => $value)
        {
            ...
            foreach($value as $v)
            {
                ...
                if($num)
                {
                    $_goodsInfo = $order_group[$v['goods']];
                    $data[] = array(
                        'goods'     => $v['goods'],
                        'goods_name' => $allgoods[$v['goods']]['name'],
                        'group'     => $key,
                        'num'       => $this -> abs ? $num : -$num,
                        'price'     => ($_goodsInfo['price']*$num*(1+$_goodsInfo['tax']/100)) * ($this -> abs ? -1 : 1),
                        'time'      => NOW,
                        'uid'       => $order['uid'],
                        'rtime'     => NOW,
        ...
        if($this -> mname == 'Sorder')
            $result = $DB_Order -> save(array('audit'=>Session::get('uid'), 'express'=>$_POST['express'], 'code'=>$_POST['code'], 'sort'=>$_POST['sort'], 'comment'=>$_POST['comment'], 'income'=>$_POST['income']*100), "`id`='{$order['id']}'");
        else
    ...
    public function getlist($condition = '')
    {
        ...
        $list = $DB_Order -> xfindAll($condition, 'id,num,cors,type,uid,time,audit,over,sale,total,income', 'time Desc', $limit);
