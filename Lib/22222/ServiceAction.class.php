    ...
    public function add()
    {
        ...
        if($_POST)
        {
            ...
            foreach($_POST['goods'] as $key => $value)
            {
                ...
                {
                    $total = inputPrice($this -> price ? $_POST['total_price'][$key] : 0);
                    ...
                    $totalPrice += $total; // + inputPrice($_POST['repair_price'][$key]);
                    $taxTotal += $_taxTotal;
                }
            }
    ...
    protected function pass()
    {
        ...
        foreach($group as $key => $value)
        {
            ...
            foreach($value as $v)
            {
                ...
                if($num)
                {
                    $_goods = $order_group[$v['goods']];
                    $data[] = array(
                        'goods'     => $v['goods'],
                        'goods_name' => $allgoods[$v['goods']]['name'],
                        'group'     => $key,
                        'num'       => $this -> abs ? $num : -$num,
                        'price'     => ($_goods['price']*$num*(1+$_goods['tax']/100)) * ($this -> abs ? -1 : 1),
                        'time'      => NOW,
                        'uid'       => $order['uid'],
                        'rtime'     => NOW,
